...
def demo(opt):
    ...
    for image_tensors, image_path_list in demo_loader:
        ...
        for img_name, pred in zip(image_path_list, preds_str):
            ...
            dict_list.append({'img_name': img_name, 'pred': pred})
    pd.DataFrame(dict_list).to_csv(f'../results/{opt.image_folder.split("/")[-2]}_{opt.saved_model.split("/")[-2]}.csv', index=False)
